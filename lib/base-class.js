...
class BaseAsync {
  ...
  register(name){
    return System.register(name, this.__process);
  }

  unregister(name){
    System.unregister(name);
  }

  async resolve(name){
    var proc = await System.resolve(name);
    if(proc){
      return proc.getValue("instance");
    }
    return null;
  }
